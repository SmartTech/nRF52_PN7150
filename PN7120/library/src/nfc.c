...
static int host_tx_rx(uint8_t *p_tx_buff, uint16_t tx_buff_len, uint8_t *p_rx_buff,
                      uint16_t rx_buff_size, uint16_t *p_nbytes_read);
                   ...
                   uint16_t *p_nbytes_read, uint16_t timeout)
{
    ...
    if( nrf_gpio_pin_read(NFC_INT_PIN) == 0 ) {
      ...
      else
      {
        while( !rx_flag ) {
            if( timer_tick > timeout ) {
                *p_nbytes_read = 0;
                NFC_LOG_INFO("wait rx_flag timeout");
                NFC_LOG_FLUSH();
                return NFC_ERROR;
            }
        }
      }
      NFC_LOG_INFO("Interrupt handled!");
      NFC_LOG_FLUSH();
    }

    if( nfc_hal_read( p_rx_buff_ptr, p_nbytes_read_ptr, rx_buff_size ) ){
        NFC_LOG_INFO("nfc_hal_read error");
        NFC_LOG_FLUSH();
        *p_nbytes_read = 0;
        return NFC_ERROR;
    }
    
    NFC_LOG_INFO("nfc_hal_read OK!");
    NFC_LOG_FLUSH();
...
static int initialize_core( void )
{
    ...
    NFC_LOG_INFO("*****Resetting and Initializing Core*****");
    NFC_LOG_FLUSH();
    /* Reset the core */
    while( host_tx_rx( CORE_RESET_CMD, sizeof( CORE_RESET_CMD ), answer, sizeof( answer ), &answer_size ) ) {
        if( !i-- ) {
          NFC_LOG_INFO("host_tx_rx trying error");
          NFC_LOG_FLUSH();
          return NFC_ERROR;
        }
        NFC_LOG_INFO("host_tx_rx error, try again!");
        NFC_LOG_FLUSH();
        nfc_hal_delay( 500 );
    }
...
{    
    ...
    nrf_delay_ms( 100 );
    NFC_LOG_INFO("*****Disabling Standby Mode*****" );              /* Note - disabling the Standby Mode should be restricted to debug sessions. */
    host_tx_rx( CORE_SET_POWER_MODE_CMD, sizeof( CORE_SET_POWER_MODE_CMD ), answer, sizeof( answer ), &answer_size );
    
    if( answer[ 3 ] == 0x00 ) {
        NFC_LOG_INFO("*****Standby OFF*****" );
        NFC_LOG_FLUSH();
    }
    
    for( i = 0, j = 4; i < 4; i++, j++ )
    {
        NFC_LOG_INFO("*****Testing Antenna*****");
        NFC_LOG_FLUSH();
    ...
    NFC_LOG_INFO("*****Starting Test Procedure*****");
    NFC_LOG_FLUSH();
    host_tx_rx( TEST_PRBS_CMD, sizeof( TEST_PRBS_CMD ), answer, sizeof( answer ), &answer_size );

    if( answer[ 3 ] == 0x00 ) {
      NFC_LOG_INFO("******Test Successful*****");
    } else {
      NFC_LOG_INFO("!!!!!!Test Failed!!!!!");
    }
    NFC_LOG_FLUSH();
    
    nfc_hal_reset();
    NFC_LOG_INFO("*****Testing Concluded Resetting*****");
    NFC_LOG_FLUSH();
#endif
    
    if( initialize_core() == NFC_ERROR) return NFC_ERROR;
...
int nfc_configure( nfc_mode_t mode )
{
    ...
    if( mode == NFC_MODE_NONE ) return NFC_SUCCESS;
...
#ifdef CARDEMU_SUPPORT
    if( mode & NFC_MODE_CARDEMU ) nci_set_config_nfca_selrsp[ 6 ] += 0x20;
#endif
